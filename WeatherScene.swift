...
class WeatherScene: SKScene, SKPhysicsContactDelegate {
    ...
    func loadSprites(){
        ...
        //let apple = addSprite(xLocation: 200, yLocation: 550, zPosition: 1, spriteFile: "apple", physicsCategory: 0b101, collidesWith: 0b1, movability: xyMovable, isCircular: true)
        ...
        let RainZone = addSprite(xLocation: 512, yLocation: 384, zPosition: 1, spriteFile: "RainZone", physicsCategory: 0b1000, collidesWith: 0b10, movability: xMovable, isCircular: false)
        ...
        let cloud = addSprite(xLocation: 1000, yLocation: 600, zPosition: 1, spriteFile: "cloud", physicsCategory: 0b10, collidesWith: 0b111, movability: xMovable, isCircular: true)
        //let rainZone = addSprite(xLocation: 512, yLocation: 384, zPosition: 1, spriteFile: "RainZone", physicsCategory: 0b1000, collidesWith: 0b10, movability: unMovable, isCircular: true)
    ...
    func cloudDidCollideWithRainZone(Cloud: SKSpriteNode, RainZone: SKSpriteNode){
        let timesUp = DispatchTime.now() + 3
        var gameTimer: Timer!
        gameTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(startRain(Cloud:)),userInfo: nil, repeats: true)
        DispatchQueue.main.asyncAfter(deadline: timesUp){
            gameTimer.invalidate()
        }
    }
    
    func startRain(Cloud: SKSpriteNode){
        //move the raindrops down the screen until they hit the bottom, and then remove them
        let raindrops = addSprite(xLocation: Double(Cloud.position.x), yLocation: Double(Cloud.position.y), zPosition: 1, spriteFile: "raindrop", physicsCategory: 0b101, collidesWith: 0, movability: unMovable, isCircular: true)
        background.addChild(raindrops)
        while(raindrops.position.y >= 0){
            raindrops.position.y -= 1
        }
        raindrops.removeFromParent()
    }
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if((firstBody.categoryBitMask == PhysicsCategory.Cloud) &&
            (secondBody.categoryBitMask == PhysicsCategory.RainZone)){
            if let Cloud = firstBody.node as? SKSpriteNode, let
                RainZone = secondBody.node as? SKSpriteNode{
                cloudDidCollideWithRainZone(Cloud: Cloud, RainZone: RainZone)
            }
        }
